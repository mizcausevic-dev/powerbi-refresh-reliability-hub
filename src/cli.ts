...
function usage(): string {
  return [
    "Usage: powerbi-refresh-reliability-hub <export.json> [options]",
    "",
    "Options:",
    "  --format <json|markdown|summary>       Output format (default: summary)",
    "  --now <iso>                             Override analysis time",
    "  --stale-remediation-after-hours <n>     Remediation SLA before stale flag (default: 24)",
...
function parseArgs(argv: string[]) {
  ...
  let staleRemediationAfterHours: number | undefined;
  ...
  while (args.length > 0) {
    ...
    switch (flag) {
      ...
      case "--now":
        now = args.shift();
        break;
      case "--stale-remediation-after-hours":
        staleRemediationAfterHours = Number(args.shift());
        break;
      case "--fail-on-high":
        failOnHigh = true;
        break;
  ...
  return { file, format, now, staleRemediationAfterHours, failOnHigh, out };
}
...
async function main() {
  const { file, format, now, staleRemediationAfterHours, failOnHigh, out } = parseArgs(process.argv.slice(2));
  const input = JSON.parse(fs.readFileSync(file, "utf8")) as PowerBIRefreshExport;
  const report = analyze(input, { now, staleRemediationAfterHours });
