...
export function toMarkdown(report: DriftReport): string {
  const lines: string[] = [];
  lines.push(report.ok ? "# Power BI refresh reliability posture ✅" : "# Power BI refresh reliability posture ❌");
  lines.push("");
  ...
  lines.push(`- Failed refresh paths: **${report.failedRefreshPaths}**`);
  lines.push(`- Delivery risks: **${report.deliveryRisks}**`);
